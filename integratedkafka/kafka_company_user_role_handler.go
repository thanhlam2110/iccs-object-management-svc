...
func KafkaCreateCompanyUseRole(c echo.Context) error {
	...
	out, err := json.Marshal(companyUserRole)
	if err != nil {
		panic(err)
	}
	kafkaStruct := new(model.KafkaStruct)
	kafkaStruct.Action = "CREATE"
	kafkaStruct.Collection = "company_user_role"
...
func GetKafkaCreateCompanyUseRoleStatus(c echo.Context) error {
	companyUserRole := new(model.CompanyUserRole)
	err := c.Bind(companyUserRole)
	if err != nil {
		fmt.Println(err)
...
func KafkaUpdateCompanyUseRole(c echo.Context) error {
	////
	companyUserRole := new(model.CompanyUserRole)
	err := c.Bind(companyUserRole)
	if err != nil {
...
func GetKafkaUpdateCompanyUseRoleStatus(c echo.Context) error {
	companyUserRole := new(model.CompanyUserRole)
	err := c.Bind(companyUserRole)
	if err != nil {
		fmt.Println(err)
